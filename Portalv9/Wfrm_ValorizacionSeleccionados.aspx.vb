...
Partial Public Class Wfrm_ValorizacionSeleccionados
...
#End Region
    ...
    Protected Sub butAceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles butAceptar.Click
        ...
        rsTransferencia = sv.ListaTransferencia_Primaria(Request.QueryString("idFolio"))
            ...
            If rsElementoArbol.Tables(0).Rows.Count > 0 Then
                ...
                If rsElementoPadre.Tables(0).Rows.Count > 0 Then
                    If rsElementoPadre.Tables(0).Rows(0).Item("idNivel") = 7 Then
                        rsElementoPadreCaja = sv.ListaArchivo_Descripciones_idDescripcion(Request.QueryString("idArchivo"), rsElementoPadre.Tables(0).Rows(0).Item("idDocumentoPID"))
                        rsElementoPadreCajaSerie = sv.ListaArchivo_Codigo_clasificacion(rsTransferencia.Tables(0).Rows(0).Item("idArchivoDestino"), rsElementoPadreCaja.Tables(0).Rows(0).Item("Codigo_clasificacion"))
                        sv.Transfiere_Archivo_Descripciones_primarias(Request.QueryString("idArchivo"), rsElementoPadre.Tables(0).Rows(0).Item("idDescripcion"), rsTransferencia.Tables(0).Rows(0).Item("idArchivoDestino"), rsElementoPadreCajaSerie.Tables(0).Rows(0).Item("idDescripcion"), gdbuscadorresultado.GetRowValues(intI, "idFolioDetalle"))
                    Else
                        rsElementoPadre = sv.ListaArchivo_Codigo_clasificacion(rsTransferencia.Tables(0).Rows(0).Item("idArchivoDestino"), rsElementoPadre.Tables(0).Rows(0).Item("Codigo_clasificacion"))
                        sv.Transfiere_Archivo_Descripciones_primarias(Request.QueryString("idArchivo"), gdbuscadorresultado.GetRowValues(intI, "idDescripcion"), rsTransferencia.Tables(0).Rows(0).Item("idArchivoDestino"), rsElementoPadre.Tables(0).Rows(0).Item("idDescripcion"), gdbuscadorresultado.GetRowValues(intI, "idFolioDetalle"))
                    End If
                End If
            End If
        Next
        ...
        For intI = 0 To gdbuscadorresultado.VisibleRowCount - 1
            sv.ABC_Transferencias_Primarias_Expedientes(3, Request.QueryString("idFolio"), gdbuscadorresultado.GetRowValues(intI, "idFolioDetalle"), gdbuscadorresultado.GetRowValues(intI, "idDescripcion"), gdbuscadorresultado.GetRowValues(intI, "idDocumentoPID"), 2)
        Next
        sv.ABC_Transferencias_Primarias(3, Request.QueryString("idFolio"), 0, Now.Date, 0, 0, "", 2)
    ...
    Protected Function Busca_HijosCaja(ByVal idDescripcion As Integer) As Boolean
        ...
        For intI = 0 To rsHijos.Tables(0).Rows.Count - 1
            rsExisteHijo = sv.ListaVencimientos_Archivo_Tramite_Seleccion_idDescripcion(Request.QueryString("idArchivo"), Request.QueryString("idFolio"), rsHijos.Tables(0).Rows(intI).Item("idDescripcion"))
    ...
    Protected Sub butCambiar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles butCambiar.Click
        Response.Redirect("Wfrm_ValorizacionSeleccionados_Upd.aspx?idArchivo=" & Request.QueryString("idArchivo") & _
    ...
    Protected Sub ASPxButton1_Click(ByVal sender As Object, ByVal e As EventArgs)
        Response.Redirect("Wfrm_ValorizacionSeleccionados_Cajas.aspx?idArchivo=" & Request.QueryString("idArchivo") & _
                                  "&idNorma=" & lblidNorma.Text & _
                                  "&idFolio=" & Request.QueryString("idFolio"))
    End Sub
End Class